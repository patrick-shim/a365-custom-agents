# Japan Expert backend infrastructure

This directory is the only backend deployment source. It contains two resource-group-scoped
deployment paths:

- `main.bicep` creates the Japan Expert backend resources and the protected MCP resource API.
...
`./tools/Test-Deployment.ps1` asserts all of this offline, plus Japan Expert naming, the absence of
...
| `azureBotDisplayName` | `Japan Expert` | Name shown to channel users. |
...
| Japan Expert Blueprint | parent identity that exposes the delegated ingress scope | Agent 365 CLI |
...
delegated ingress scope read back from the newly registered Japan Expert Blueprint after the Agent 365
...
record is historical only. It is not Japan Expert deployment authority, and no Seoul name, identity,
