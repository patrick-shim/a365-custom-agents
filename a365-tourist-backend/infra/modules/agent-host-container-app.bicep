...
var productionEnvironment = [
  ...
  {
    // ServiceConnection is the ConnectionsMap default for outbound channel calls on /api/messages.
    // Those calls target the Agent 365 Messaging Bot API, which is the audience the Blueprint holds
    // AgentData.ReadWrite against. Agentic handlers do not use this value: the SDK's agentic token
    // provider hardcodes the Entra token-exchange audience for its federated legs.
