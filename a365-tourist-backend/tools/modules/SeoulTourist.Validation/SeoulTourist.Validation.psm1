...
function Test-StaRepositoryConfiguration {
    ...
    $programPath = Join-Path $root 'server\agent-host\SeoulTourist.AgentHost\Program.cs'
    $applicationPath = Join-Path $root 'server\agent-host\SeoulTourist.AgentHost\SeoulTouristApplication.cs'
    $hostSettingsPath = Join-Path $root 'server\agent-host\SeoulTourist.AgentHost\appsettings.json'
    $oboExchangePath = Join-Path $root 'server\agent-host\SeoulTourist.AgentHost\AgentIdentityOboTokenExchange.cs'
    $frontendIdentityBindingPath = Join-Path $root 'server\agent-host\SeoulTourist.AgentHost\AgentFrontendIdentityBinding.cs'
    $internalMcpOptionsPath = Join-Path $root 'server\agent-host\SeoulTourist.AgentHost\InternalMcpOptions.cs'
    $internalMcpCatalogPath = Join-Path $root 'server\agent-host\SeoulTourist.AgentHost\InternalMcpToolCatalog.cs'
    $authorizationTestsPath = Join-Path $root 'tests\SeoulTourist.AgentHost.Tests\AgentIdentityAuthorizationOptionsTests.cs'
    $oboExchangeTestsPath = Join-Path $root 'tests\SeoulTourist.AgentHost.Tests\AgentIdentityOboTokenExchangeTests.cs'
    $frontendIdentityBindingTestsPath = Join-Path $root 'tests\SeoulTourist.AgentHost.Tests\AgentFrontendIdentityBindingTests.cs'
    $observabilityTokenCacheTestsPath = Join-Path $root 'tests\SeoulTourist.AgentHost.Tests\ObservabilityTokenCacheTests.cs'
    $internalMcpCatalogTestsPath = Join-Path $root 'tests\SeoulTourist.AgentHost.Tests\InternalMcpToolCatalogTests.cs'
    $mcpAuthorizationPath = Join-Path $root 'server\mcp\shared\SeoulTourist.Mcp.Hosting\McpWorkloadAuthorization.cs'
    $mcpAuthorizationTestsPath = Join-Path $root 'tests\SeoulTourist.Mcp.Hosting.Tests\McpWorkloadAuthorizationTests.cs'
    ...
    $hostSourceDirectory = Join-Path $root 'server\agent-host\SeoulTourist.AgentHost'
    ...
    $readmeValid = $readmeFirstLine -eq '# Seoul Tourist Assistant Backend'
    $results.Add((New-StaValidationResult -Area 'Repository' -Check 'README identity' `
        ...
        -Remediation $(if ($readmeValid) { '' } else { 'Replace README.md with the Seoul Tourist Assistant Backend documentation.' })))
...
function Test-StaPurviewReadiness {
    ...
    if ($isImplemented) {
        ...
        $purviewTestDirectory = Join-Path ([IO.Path]::GetTempPath()) "seoul-tourist-purview-tests-$([guid]::NewGuid())"
        ...
        try {
            [Environment]::SetEnvironmentVariable(
                ...
                $root,
                [EnvironmentVariableTarget]::Process)
            $purviewTestProject = Join-Path $root 'tests\SeoulTourist.AgentHost.Tests\SeoulTourist.AgentHost.Tests.csproj'
    ...
    if ($ProbePolicy) {
        ...
        if ([string]::IsNullOrWhiteSpace($token) -or [string]::IsNullOrWhiteSpace($UserId) -or [string]::IsNullOrWhiteSpace($ApplicationId)) {
            ...
            $entry = [ordered]@{
                ...
                name            = 'Seoul Tourist validation probe'
            ...
            $requestBody = [ordered]@{
                contentToProcess = [ordered]@{
                    ...
                    integratedAppMetadata = [ordered]@{
                        name = 'Seoul Tourist Assistant validation tools'
                        version = '1.0'
                    }
                }
            }
...
function Invoke-StaLocalCi {
    ...
    $root = Resolve-StaRepositoryRoot -RepositoryRoot $RepositoryRoot
    $results = [System.Collections.Generic.List[object]]::new()
    $solutionPath = Join-Path $root 'SeoulTouristAgent.slnx'
    $runDirectory = Join-Path ([IO.Path]::GetTempPath()) "seoul-tourist-ci-$([guid]::NewGuid())"
    ...
    $services = @(
        @{
            Name       = 'Agent host'
            ProjectDir = 'server\agent-host\SeoulTourist.AgentHost'
            Assembly   = 'SeoulTourist.AgentHost.dll'
            HealthChecks = @(
        ...
        @{
            ...
            ProjectDir = 'server\mcp\attractions\SeoulTourist.Mcp.Attractions'
            Assembly   = 'SeoulTourist.Mcp.Attractions.dll'
            HealthChecks = @(@{ Name = 'Attractions MCP health'; Path = '/health'; ExpectedStatus = @('healthy') })
        },
        @{
            Name       = 'Weather MCP'
            ProjectDir = 'server\mcp\weather\SeoulTourist.Mcp.Weather'
            Assembly   = 'SeoulTourist.Mcp.Weather.dll'
            HealthChecks = @(@{ Name = 'Weather MCP health'; Path = '/health'; ExpectedStatus = @('healthy') })
        },
        @{
            Name       = 'Accommodation MCP'
            ProjectDir = 'server\mcp\accommodation\SeoulTourist.Mcp.Accommodation'
            Assembly   = 'SeoulTourist.Mcp.Accommodation.dll'
            HealthChecks = @(@{ Name = 'Accommodation MCP health'; Path = '/health'; ExpectedStatus = @('healthy') })
        },
        @{
            Name       = 'Currency MCP'
            ProjectDir = 'server\mcp\currency\SeoulTourist.Mcp.Currency'
            Assembly   = 'SeoulTourist.Mcp.Currency.dll'
