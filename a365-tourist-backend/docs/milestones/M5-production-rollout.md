...
> plan. Active M7 owns any fresh deployment, package, consent, policy, and live-acceptance work under
> its reviewed dry-run, rollback, and explicit-approval gates; this historical M5 record grants no
> authority by itself.
...
- The user explicitly authorized the full live deployment on 2026-08-16 after M6 extracted the
  canonical backend.
- Deployment `koreaexpert-m5-backend-20260816-01` succeeded after fresh backend-specific validation.
...
- The AI Teammate version `1.1.5` publication recorded below was a separately user-authorized,
  ...
  does not authorize future package operations; active M7 requires a fresh dry run and approval.
...
- Added and compiled `infra/m5-container-apps-update.bicep` and its typed parameter file. The wrapper
  references all dependencies as existing and deploys only the five Container App modules.
...
- Deployment `koreaexpert-m5-hardening-20260811-01` succeeded. The host and four MCP apps run healthy,
...
- Deployment `koreaexpert-m5-backend-20260816-01` succeeded. The shared host and four MCP apps run
...
- The `KoreaExpert Blueprint` AI Teammate package was generated by the supported CLI at version
