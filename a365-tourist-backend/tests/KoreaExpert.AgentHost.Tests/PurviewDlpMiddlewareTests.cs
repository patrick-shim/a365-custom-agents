...
public sealed class PurviewDlpMiddlewareTests
{
    ...
    public void PurviewConversationIncludesAgentIdentityMetadata()
    {
        ...
        Assert.AreEqual("Korea Tourist Assistant Test", agent.GetProperty("name").GetString());
    ...
    {
        PurviewSerializationWorkaround.Apply();
        var settings = new PurviewSettings("Korea Tourist Assistant Test")
        {
            AppVersion = "1.0",
    ...
    private static string FindRepositoryRoot()
    {
        ...
        if (!string.IsNullOrWhiteSpace(configuredRoot)
            && File.Exists(Path.Combine(configuredRoot, "KoreaExpertAgent.slnx")))
        {
            return configuredRoot;
        }

        var directory = new DirectoryInfo(AppContext.BaseDirectory);
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "KoreaExpertAgent.slnx")))
        {
