# Japan Tourist Expert AI Teammate Frontend Guide
...
- `assets/color.png` and `assets/outline.png` are the committed source-owned Japanese-flag icons for
  clean Japan Tourist Expert (Teammate) package generation.
...
- Existing CLI-owned manifest and published package records are historical Seoul state. Rehydrate a
  new Japan Tourist Expert (Teammate) package through the approved Agent 365 workflow, never a source edit.
...
Label source-safe metadata Japan Tourist Expert (Teammate) while preserving
`TokenValidation__Audiences__AgenticUser`, `/api/messages`, one shared Blueprint with two children,
and dynamic child selection. Prepare a clean Agent 365 workflow for Japan Tourist Expert BP and
