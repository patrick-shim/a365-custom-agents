# Japan Tourist Expert AI Teammate frontend
...
is not M8 input. Rehydrate a new Japan Tourist Expert (Teammate) package through the approved Agent 365 workflow and
...
The source-safe package boundary carries Japan Tourist Expert (Teammate) branding and Japanese-flag icons. The clean
Agent 365 run reused the single shared Blueprint, now named `Japan Tourist Expert BP`, without reading or reusing Seoul
...
default icons, so the generated package was completed with Japan Tourist Expert naming, the Japanese red
accent, and the source-owned icons. The Admin Center lists an uploaded agent by `name.short`, which is
`Japan Tourist Expert (Teammate)` so administrators can distinguish it from the `Japan Tourist Expert (OBO)` Teams app.
The full Teammate name has 31 characters; use `Japan Tourist Expert (Team)` (27 characters) in
`name.short` to satisfy the Teams 30-character limit.
