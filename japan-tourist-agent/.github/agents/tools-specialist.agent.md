...
deployment authority. M8 prohibits restoring frontend backend mirrors.
...
2. Read `a365-tourist-backend/tools/README.md` and reuse the backend
   `JapanExpert.Validation.psm1` rather than duplicating logic.
...
- For M8 Purview tenant reads, call `Get-FeatureConfiguration` with the semantic
