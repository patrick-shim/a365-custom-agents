...
public sealed class AgentChatClientRegistryTests
{
    ...
    [TestMethod]
    [TestCategory("Purview")]
    public void BoundsRetainedClientsAndDisposesEvictedClientsOnce()
    {
        var clients = new List<DisposableChatClient>();
        var registry = new AgentChatClientRegistry(
            applicationId =>
            {
                var client = new DisposableChatClient(applicationId);
                clients.Add(client);
            ...
            retainedClientCapacity: 2);

        registry.Create("11111111-1111-4111-8111-111111111111");
        registry.Create("22222222-2222-4222-8222-222222222222");
        registry.Create("33333333-3333-4333-8333-333333333333");

        Assert.AreEqual(1, clients[0].DisposeCount);
        Assert.AreEqual(0, clients[1].DisposeCount);
        Assert.AreEqual(0, clients[2].DisposeCount);
