...
public sealed class PromptShieldOptions
{
    ...
    public string BlockedPromptMessage { get; init; } =
        "Your request was blocked because it looks like an attempt to manipulate the agent's instructions. Error code: JEX-SHIELD-001.";

    [Required]
    public string BlockedDocumentMessage { get; init; } =
        "A tool returned content that looks like an embedded instruction attack, so the turn was stopped. Error code: JEX-SHIELD-002.";

    [Required]
    public string EvaluationFailureMessage { get; init; } =
        "I could not safely screen this request for prompt-injection attempts. Please try again later. Error code: JEX-SHIELD-003.";
