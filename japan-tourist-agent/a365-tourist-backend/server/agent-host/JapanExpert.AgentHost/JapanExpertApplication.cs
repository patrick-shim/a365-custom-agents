...
public sealed partial class JapanExpertApplication : AgentApplication
{
    ...
    private readonly ILogger<JapanExpertApplication> _logger;
    private readonly bool _isLocalEnvironment;

    public JapanExpertApplication(
        ...
        ILogger<JapanExpertApplication> logger)
        : base(options)
    {
        ...
        foreach (var member in turnContext.Activity.MembersAdded ?? [])
        {
            if (member.Id != turnContext.Activity.Recipient?.Id)
            {
                await turnContext.SendActivityAsync(
                    MessageFactory.Text("Hello. I can help plan a trip anywhere in Japan around your schedule, the weather, and your preferences."),
                    cancellationToken);
            }
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await turnContext.SendActivityAsync(
                MessageFactory.Text("Tell me what you would like to plan in Japan."),
                cancellationToken);
            return;
        }
        if (inputRejection == AgentInputRejection.TooLong)
        {
            ...
            await turnContext.SendActivityAsync(
                MessageFactory.Text(
                    "Your request is too long. Shorten it and try again. Error code: JEX-INPUT-001."),
                cancellationToken);
            return;
        }
        ...
        // turn; the bounded factory registry retains wrappers past turn completion so background
        // audit work can finish without sharing policy state across requests.
        var chatClient = _chatClientFactory.Create();
        var agent = JapanExpertAgentFactory.Create(
            chatClient,
        ...
        Guid? authenticatedUserId)
    {
        ...
        var agentDetails = new AgentDetails(
            agentId: turnTokens.AgentId,
            agentName: "Japan Tourist Expert",
        ...
        string? preferredMessage = null)
    {
        ...
        var dependencyStatus = AgentFailureClassifier.GetDependencyStatus(exception);
    ...
    [LoggerMessage(
        EventId = 1101,
        Level = LogLevel.Warning,
        Message = "Turn operation {Operation} failed safely: code={FailureCode}, kind={FailureKind}, frontend={FrontendMode}, exceptionType={ExceptionType}, dependencyStatus={DependencyStatus}.")]
    ...
    [LoggerMessage(
        EventId = 1102,
        Level = LogLevel.Error,
        Message = "Turn operation {Operation} failed unexpectedly: code={FailureCode}, kind={FailureKind}, frontend={FrontendMode}, exceptionType={ExceptionType}, dependencyStatus={DependencyStatus}.")]
