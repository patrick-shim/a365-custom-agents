...
> M3`. M3 remains blocked, M7 is historical, and active M8 owns the Japan Expert migration and fresh
> cross-channel acceptance. M3's remaining checklist is preserved for later resumption. Production
> deployment and tenant mutation remain outside M3.
...
- [x] Keep automatic retry disabled; introduce bounded retry with jitter only for a proven
      idempotent operation with explicit retry-guidance tests.
