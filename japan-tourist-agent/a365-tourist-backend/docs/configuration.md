...
An owning frontend may run an M8-authorized Agent 365 workflow only after a reviewed dry run,
...
Production defines three `AuthType=FederatedCredentials` connection profiles:

- `ServiceConnection`: `ClientId` is the shared Blueprint application ID and `FederatedClientId` is
  the host Container App UAMI client ID. `ConnectionsMap` selects it for the Agentic User audience,
  so it authenticates outbound `/api/messages` channel calls; its scope is therefore the Agent 365
  Messaging Bot API, `5a807f24-c9de-44ee-a3a7-329e88a00ffc/.default`, which is the audience the
  Blueprint holds `AgentData.ReadWrite` against. Agentic handlers do not consume this scope: the SDK
  agentic token provider hardcodes the Entra token-exchange audience for its federated legs and then
  requests each handler's own resource scope. It has no static `AgentId`; Agentic User auth uses the
  signed activity child.
- `OboServiceConnection`: uses the same Blueprint and UAMI values to obtain the child-bound parent
  token, with `api://AzureADTokenExchange/.default` as its federation scope.
  `AgentIdentityObo.AgentId` supplies the OBO child Agent Identity as `fmi_path` and the child OBO
  `client_id`; the connection itself has no `AgentId` setting.
- `OboChannelConnection`: `ClientId` is the OBO channel application ID,
  `FederatedClientId` is the host UAMI client ID, and its scope is
  `https://api.botframework.com/.default`. `ConnectionsMap` selects it for the OBO audience so Bot
  Connector replies use the channel application rather than the shared Blueprint.
...
confidential-client FMI, OBO, or channel exchanges required here.

## Agent host

| Environment variable | Purpose |
| --- | --- |
| `AgentHost__FoundryProjectEndpoint` | Existing Foundry account OpenAI-compatible endpoint; currently `https://a365-ai-foundry.services.ai.azure.com`. The host appends `/openai/v1`. The project-scoped `/api/projects/<name>` form does not publish that surface and must not be used. |
| `AgentHost__FoundryModelDeployment` | Responses API model deployment; currently `gpt-5.6-sol`. |
| `AgentHost__MaximumPromptCharacters` | Maximum accepted prompt length; default `32768`. Oversized input returns `JEX-INPUT-001`. |
...
| `InternalMcp__MaximumContentCharacters` | Maximum MCP tool argument or result length evaluated by the content protector; default `65536`. |
...
| --- | --- |
| `agentic-foundry` | `https://ai.azure.com/.default` |
...
The OBO Azure Bot OAuth connection defaults to the non-secret name `japan-tourist-assistant-obo`. The
phase-gated Bot module creates it as Aadv2, with the separate OBO channel application as its client
and the fresh Blueprint delegated ingress scope supplied after registration; its credential crosses
the templates only as a secure parameter. Do not configure automatic `OBOConnectionName` or
`OBOScopes` on `obo-user`; doing so would mint downstream tokens as the Blueprint. The host uses
`OboServiceConnection` to obtain the child-bound parent token and then performs the child OBO exchange
explicitly. Confirm the live scope name and connection in the setup dry run and Teams/Azure Bot
configuration; do not invent or commit tenant identifiers or secrets.
...
content-activity completion, caps retained wrappers at 4096, disposes the oldest wrapper when the cap
is exceeded, and disposes every remaining wrapper exactly once at application shutdown. The cap is
intentionally above the supported per-replica concurrent-turn envelope; exceeding 4096 simultaneous
turns on one replica is unsupported because eviction could otherwise reach an active wrapper. Turn
code must neither cache nor dispose it.
...
and return a message carrying an error code (`JEX-SHIELD-001` for a blocked prompt, `-002` for a
...
2. Agent 365 CLI 1.1.214 does not assign Foundry Azure RBAC. After each child exists, grant its
   ...
   though this host never uses keys; replace it only with an inference-only custom role proven
   against this exact Responses deployment. Do not grant either role to the host UAMI.
...
4. Configure and test the `japan-tourist-assistant-obo` Azure Bot OAuth connection against the shared
...
The active attractions and accommodation MCP services query OpenStreetMap through a configurable
Overpass interpreter. No Azure Maps account, workload data-plane role, or subscription key is
provisioned or required, and no provider secret is deployed. The deployment supplies the non-secret
`Overpass__Endpoint` and `Overpass__UserAgent` settings; endpoints, the `User-Agent` contact
reference, and the JMA, MET Norway, Frankfurter, and ECB addresses are deployment parameters
documented in [`infra/README.md`](../infra/README.md).

## Weather

The Japan Meteorological Agency is authoritative for forecasts and alerts. Forecast requests use
the guarded JMA forecast JSON endpoint and fail closed on legacy, stale, or malformed schemas. Alerts
come from the documented JMA XML Atom feed and link to the authoritative bulletin. The tool relays
and attributes JMA content; it never authors an unofficial Japanese warning.

MET Norway supplies only clearly labelled third-party current conditions. It is not represented as a
JMA observation, forecast, or warning. Both providers use bounded response sizes, timeouts, and
in-process caches.
...
The currency MCP uses credential-free published reference rates. Frankfurter v2 is pinned to the
European Central Bank provider; direct ECB SDMX is the fallback. JPY is the default target, rate
observation dates and attribution are returned, and all conversion arithmetic is local.
...
Accommodation uses the same credential-free Overpass pattern as attractions. It reads
`Overpass__Endpoint` and `Overpass__UserAgent` and requires no Azure data-plane role, subscription
