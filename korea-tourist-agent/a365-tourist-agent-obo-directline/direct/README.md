...
- The Bot Token Service connection is named `korea-tourist-assistant-obo`.
...
Read `KOREA_EXPERT_DIRECT_LINE_SECRET` from the Azure Bot's Direct Line channel keys and set it only
...
| `--user-name <name>` | `Korea Tourist Expert CLI` | Display name sent with activities |
