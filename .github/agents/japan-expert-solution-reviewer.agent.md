---
name: "Japan Expert Solution Reviewer"
description: "Use when reviewing the Japan Expert solution for correctness, security, Agent 365 integration risks, MCP contract mismatches, Teams boundary issues, or missing tests."
...
- Review this shared backend as canonical. Flag any proposed backend change sourced from any
  channel-only frontend or any M8 change that restores backend residue outside the backend child.
...
- Verify claims against pinned package versions, the canonical contract, and the current M8 evidence
